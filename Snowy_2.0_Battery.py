...
n = 105120  # number of data sets used 

df = pd.read_csv('full_data.csv', names=colnames, nrows=n)  # change this to match csv file
...
m = 15 # lifetime
f_hydro = 0 #100000000 * (k * (1+k) ** m ) / ((k+1)**m -1)  # capital + O&M ($/yr)
v_hydro = 0  # SRMC ($/MWh)
cap_hydro = 150  # installed capacity (MW)
print(f_hydro)
# ---------------------------------
...
strike_price = 20   # price that hydro will charge
discharge_price = 300  # price that hydro will discharge
strike_price_step = 5
...
while discharge_price <= 300:  # & strike_price <= 40:
    for i in range(n):
        if i > 0:  # exclude the first row of data
            ...
            if price_list[i] >= strike_price:  # if spot price > 10
                ...
                storage_soc[i] = storage_soc[i - 1] + (stored_hydro[i] * (
                            time_diff / min_in_hour))  # keep SOC as MWh for now, we'll normalise it later
    ...
    profit_hydro = 0.85 * (24 * 365 * sum(op_profit_hydro * 12) / n - f_hydro) / 1000  # X 12 cos op profit hydro was in $/5min unit
